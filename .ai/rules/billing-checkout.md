...
## `SubscriptionFactory` must extend Laravel's `Factory`, and must override `modelName()`
Two separate traps, both silent, both learned the hard way and both now recorded here rather than in a 12-line docblock on the factory.

- **Extending Cashier's `SubscriptionFactory` loses the model's static type.** Cashier's declares no `@extends Factory<...>`, so anything inheriting it resolves its model as the bare `Model`, and every call passing `Subscription::factory()->make()` to a `Subscription` parameter becomes a level-9 error. Extending `Illuminate\Database\Eloquent\Factories\Factory` with `@extends Factory<Subscription>` gives up only Cashier's unused state helpers.
- **`modelName()`, never a `$model` property.** The inherited `protected $model = \Laravel\Cashier\Subscription::class` short-circuits `Factory::modelName()`, so fixtures build as Cashier's model, which does not compose stancl's `CentralConnection`. The row lands on the *default* connection inside `RefreshDatabase`'s open transaction while the code under test reads `central`, then inserts its own row with the same unique `stripe_id` and blocks on the uncommitted duplicate key. It surfaces as `SQLSTATE 1205 Lock wait timeout`, not as a wrong-model error.

## Usage metering: the identifier is the whole safety story
Added 2026-09-17 with `.claude/plans/archive/usage-metering.md`.

- **A meter event's identifier must derive from the counter's cumulative total, not from the period.** Stripe deduplicates on `identifier`, so one identifier per tenant/meter/period would bill the first increment and silently drop every later one in that period. `Actions\Billing\Usage\ReportTenantUsage::identifier()` hashes tenant, event name, period start *and* the running total, and each run sends the delta; `tenant_usage.reported_value` is what the next delta is measured from. Send first, mark second: a failure after the send is recovered by the next run recomputing the same identifier, while marking first loses the usage outright.
- **`Cashier::reportMeterEvent()` generates a random identifier and its `$options` spread lands after it**, so passing `['identifier' => …]` overrides the UUID. Reaching for `Cashier::stripe()->v2->billing->meterEvents` directly is unnecessary.
- **`meter_id` and `meter_event_name` are columns on `subscription_items`, not on `subscriptions`** — and so is Stripe's billing period, which moved onto the item in the current API. `GetBillingPeriod` reads the stamped item and falls back to the subscription's own anniversary (yearly when the price is the plan's `yearly_id`), never to a calendar month: a plan bought on the 9th bills the 9th to the 9th.
- **Cashier's own webhook handlers write the item rows and none of those four columns.** `SyncMeteredItems` runs after `parent::` in both `handleCustomerSubscriptionCreated` and `handleCustomerSubscriptionUpdated`, and `StampSubscriptionMeters` supplies the event name from plan metadata, because Stripe carries the meter id and the plan carries the name.
- **A metered capability's `included` allowance reads as its limit but never refuses `Entitlements::consume()`** — the overage is what the plan sells. Metered keys also bucket by billing period while quota keys stay lifetime counters, which is why `PlanEntitlements` resolves a period per capability rather than once per tenant.

## Promotion codes: Stripe owns the arithmetic, and the wrappers carry no types
Added 2026-09-17 with `.claude/plans/archive/coupons-and-promotions.md`.

- **Never `findActivePromotionCode()` for validation.** Its `active=true` filter turns an expired code into "no such code", so the customer is told to check a spelling that was right. `ValidatePromotionCode` lists unfiltered and decides why itself, one `PromotionCodeUnavailable::<reason>()` per failure mode.
- **Cashier's `PromotionCode`, `Coupon` and `Discount` expose Stripe's fields through `__get()`**, which is a level-9 error at every read. Go through `asStripePromotionCode()` / `asStripeCoupon()` / `asStripeDiscount()`, whose `@property` annotations are typed.
- **The current Stripe API keeps a discount's coupon at `discount.source.coupon` and a promotion code's at `promotion_code.promotion.coupon`.** `Cashier\Enums\StripeApiVersions::current()` picks the shape, so a test fixture written against `discount.coupon` fails with "Attempt to read property coupon on null" rather than anything naming the version.
- **The promotion code id is never stored.** `tenant_provisions.promotion_code` holds the typed code and `CreateInlineSubscription` resolves the id again, because a redemption limit moves between the page load and the charge. A code that fails that second check is dropped (logged, column cleared) rather than refusing a sale the customer already asked for.
- **`applied_promotions` is an audit trail, not a source of truth.** Whether a discount still runs is `GetTenantDiscount`, which reads Stripe and returns null on an API error — a screen that renders a discount during an outage is worse than one that omits it.
- **A Livewire public property typed as a `spatie/laravel-data` object needs `Concerns\WireableData`**, or the component dies with "Property type not supported in Livewire" from the view render, not from the assignment.
- **A tenant-scoped query with a nullable id is a leak.** The staff panel's applied-promotion list matches `stripe_subscription_id` only when there is one: `orWhere('stripe_subscription_id', null)` matches every unlinked row, which is every other tenant's redemptions.
