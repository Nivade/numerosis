...
Docblocks are where the volume is. `src/` holds 2,145 lines of docblock prose against 366 inline comment lines, and 270 of its 300 em-dashes. Keep `@param`, `@return` and array shapes exactly as they are. Prose inside a docblock follows the same rules as a `//` comment.

For the prose itself, use the `no-ai-slop` skill (`~/.claude/skills/no-ai-slop`). Its pattern catalog is the standard here, and it covers more than a hand-rolled list will. The four patterns this repo measurably fails on:

| Pattern | Count in `src/` | Example |
| --- | --- | --- |
| Em dashes | 300 | `has not run — a discovered satellite provider` |
| Binary contrast | 95 `rather than`, 53 `, not ` | `Non-string schema names are dropped rather than coerced` |
