...
| `src/Support/Cache/**`, `src/Providers/TenancyServiceProvider.php` | [tenant-caching.md](tenant-caching.md) — `global_cache()` un-prefixed; cross-tenant leaks of cached tenant models + shared session guard keys; `cache.serializable_classes` silently kills the domain resolver cache. |
