...
| `src/Models/**`, `src/Features/**`, `src/Support/Compat/**`, `composer.json` | [optional-dependencies.md](optional-dependencies.md) — eager `implements`/`use trait` vs lazy type-hints; `Support\Compat\*`; one `class_exists` seam per package; absence only testable in a subprocess. |
...
| `src/NumerosisServiceProvider.php`, `src/Support/HostConfig.php`, `src/Support/Domains.php` | [package-host-bootstrap.md](package-host-bootstrap.md) — `Domains.php` can't call facades; host `bootstrap/app.php`/`providers.php` staleness; `HostConfig::apply()` register-vs-booting race; `Numerosis::middleware()` fatal on a real (non-Testbench) boot. |
| `src/**Tenancy**`, `config/numerosis/tenancy.php` | [stancl-tenancy-v4.md](stancl-tenancy-v4.md) — **port map, not current code**: v3-only since 2026-08-31; 11 moved symbols (9 eager) + 4 moved config keys; v4 docs wrong on 3 points. |
...
| `src/Policies/**`, `src/Http/Controllers/**`, `routes/tenant.php` | [central-rows-on-tenant-routes.md](central-rows-on-tenant-routes.md) — binding a `CentralConnection` model on a tenant route has no tenant scope and `deleteAny` is seeded to every tenant's admin; scope in the policy. Signed-link/row expiry interplay. |
...
| `src/Resolvers/**`, `config/numerosis/tenancy.php` | [identification-modes.md](identification-modes.md) — subdomain/custom-domain/path; slug ≠ domain; `{tenant}` literal enables dots; all three modes now browser-tested. |
...
| `**/*.php` | [general.md](general.md) — comment style: default to zero, no em-dash/"rather than" cadence, never cite a `.claude/plans/*` file; docblocks are the bulk. Validate preservation when shortening; `unslop` itself is a no-op here. |
