...
**The plan is complete.** What is deliberately *not* done, and where it is
recorded rather than lost:

- the rest of E's smoke path (subdomain / custom-domain wizard legs, a module
  purchase, the admin panel showing the new tenant) — section E
- `DesignLanguageGuardTest`'s heroicon scan is vacuous: it scans core `src/`
  only, and every Filament resource is in `packages/filament` — section C, and
  `.claude/rules/package-split.md`
- the PHPStan baseline's environment sensitivity is explained but not fixed;
  the baseline was deliberately left un-regenerated — section B
