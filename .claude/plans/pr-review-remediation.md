...
| 2 OAuth null email | **Fixed.** Maintainer took option 2: `users.email` is nullable on both connections. Downstream audit done by making the `@property` nullable on all three user models and running `analyse` cold — three real sites surfaced (`UserAccountDeleted::$email`, `AcceptInvitation`'s `Str::lower()`, `Profile::$email`) and two more were found by reading (`sendEmailVerificationNotification()` would mail a null address; `StoreInvitationRequest` was already safe). Second defect fixed: `refreshTokens()` writes the new `SocialUserData::credentialAttributes()`, not the wide array. Third verified rather than assumed — GitHub's `/user` payload has no `email_verified` key, so that arm was dead; it now reads `$user->getEmail() !== null`, sound because Socialite's driver defaults to `user:email` and returns only a `primary && verified` address. Three regression tests in `SocialLoginTest`, all confirmed failing first with `SQLSTATE[23000] … Column 'email' cannot be null`. `.ai/rules/auth-login.md` updated with the nullable invariant and the corrected GitHub claim. Suite 652 passed / 6 skipped; PHPStan clean cold; baseline still 5 |
| 3 `AcceptInvitation` transaction | **Fixed,** and the phase understated it. Both named fixes shipped: `MemberJoined`/`MemberRemoved` implement `ShouldDispatchAfterCommit`, and the observer's tenant-database write moved into `$membership->getConnection()->afterCommit()` (the observer, not the caller, so every attach path is covered and `AddTenantOwner`'s no-transaction case still runs inline). **A third defect turned up underneath:** `AcceptInvitation` took its transaction with `DB::transaction()`, which opens on `database.default` (`mysql`), while every model in it is `CentralConnection` (`central`) — two PDO handles, two transaction stacks, so the transaction covered none of its own writes and the atomicity its docblock claims did not exist. Now `$invitation->getConnection()->transaction()`. Two regression tests, both confirmed red against the unfixed observer/events. Getting them honest took two rewrites — see the new `.ai/rules/testing.md` section on `RefreshDatabase` firing after-commit callbacks at level 1, and on a test-registered `created` listener running before the package observer. Suite 654 passed / 6 skipped; PHPStan clean cold; baseline still 5 |
| 4 webhook payload guard | **Fixed** 2026-09-14, and larger than the phase described. The expanded-`customer` crash is not in this controller's reads at all — Cashier's own `handleCustomerSubscriptionCreated`/`Updated`/`Deleted`, `handleCustomerUpdated`, `handlePaymentMethodAutomaticallyUpdated` and both invoice handlers pass the field straight to `Cashier::findBillable()`, which is typed `Stripe\Customer|string|null`, so a 500 happens *before* any override runs. Guarding six call sites would have left `payment_method.automatically_updated` unguarded, so the payload is flattened once in an overridden `handleWebhook()` instead. `current_period_end` is confirmed moved: `vendor/stripe/stripe-php` pins `2026-08-26.dahlia`, its `Subscription` carries no such property and `SubscriptionItem` does, so the grace period has been silently null on every cancellation — now read from `items.data[0]` with the top-level key as the pre-`basil` fallback. The multi-item skip is now stated rather than implied. Three regression tests in `WebhookControllerLifecycleTest`, all confirmed red first (two as literal `TypeError` 500s). Suite 725 passed / 6 skipped; PHPStan clean cold; baseline still 17 |
| 5.1 line-number citations | **Closed** 2026-09-14. Every `path.php:NNN` in `.ai/rules/` is now the enclosing symbol, including three the 2026-09-07 audit did not list (`tenant-provisioning.md` twice, into `vendor/stancl/`, and `exception-handling.md`). `tests/Feature/Docs/RuleCitationsTest.php` fails on any new one — confirmed red against the old text. Previously: **Partial, 7 of 9 left.** `middleware-registration.md`'s two are gone. Still live: `tenant-filesystem.md:12`/`:15`, `tenant-registration-wizard.md:25`/`:27`, `package-boundaries.md:57` (now citing the renamed `FeatureRegistry.php:127`)/`:64`, `host-integration-quickstart.md:130` |
| 5.2 two alias registries | **Closed** 2026-09-14. The wrong bullet is replaced by the constraint that actually holds: `middlewareAliases()` returns class-string literals only, because it runs before `RegisterFacades`, and `makeTenancyMiddlewareHighestPriority()` must name both delegating classes since Laravel's priority sort matches the alias's literal target. Previously: **Partial, and the leftover half was actively wrong.** `middleware-registration.md` gained a correct "One alias registry" bullet naming `registerMiddleware()`. But the bullet below it still says `tenancy.identification`/`tenancy.route` "resolve through `TenancyServiceProvider`, not to a class literal" — Phase 1 made them class literals (`InitializeTenancy::class`, `TenantRouteGuard::class`), which is the opposite claim, and the new trap (this method is evaluated before facades bind, so literals *only*) is stated in `Numerosis.php`'s docblock and nowhere in the rules |
| 5.3 plans README | **Closed.** The three wrong Live rows were fixed in the 2026-09-12 re-sort, and the `--parallel` Abandoned row is true again now that `composer test` is parallel. This plan's own row was the last stale one and is rewritten. Previously: **Partial.** Re-sorted 2026-09-07: `config-consolidation.md` and `simplification-followups.md` archived. Two Live rows still wrong — `domain-events-expansion.md` says "Confirmed unstarted: no `src/Actions/Tenancy/EnsureTenantUserExists.php`" (that file exists), `invitations-social-redesign.md` says "Not executed" (`Models/Central/{Invitation,SocialAccount}`, `Enums/Auth/SocialProvider`, two migrations and three controllers all ship). The Abandoned row still says `--parallel` was dropped; `composer.json:106` is `pest --parallel` |
| 5.4 deleted-config instructions | **Closed** 2026-09-14. `invitations-social-redesign.md` was archived in the 2026-09-12 re-sort. `post-extraction-review.md`'s 4.3 bullet now records that the key shipped and was then deleted by the config consolidation, rather than reading as available to build on |
| 5.5 `comment-destyle.md` status | **Closed** 2026-09-14. Its Status heading now carries the 2026-09-14 measurement (36 and 10 in `src/`, 56 and 35 repo-wide) and says the table below records what the sweep left, not the tree. The file is archived, so it is a record either way; what it must not do is read as current |
| 6.1 over-cap docblocks | **Closed** 2026-09-14: zero repo-wide, from 56. Most of the excess was history (which shape a seam replaced, which older design it superseded), which git carries; the host-facing material was already in `docs/extending.md`. Three facts had no other home and moved to `.ai/rules/`: the install doctor's `VERIFIED_CONFIG_KEYS` drift, and both `SubscriptionFactory` traps. Previously: **Open, and the scope had grown ~10x.** The two originals stand (`HostConfig` 9 with the decorative bold intact, `TenantRegistrationData` 7). Repo-wide there are now **37**, worst first: `packages/ui/.../toast.blade.php` 16, `RoleAndPermissionSeeder` 14, `NumerosisUiServiceProvider` 13, `Contracts/Billing/BillableUser` 12, `GeneratesUniqueEmails` 12, `SubscriptionFactory` 12, `Numerosis.php:335` 11, `ConfiguredSteps` 10, `PaymentPlanSeeder` 10 |
| 6.2 `//` runs over cap | **Closed** 2026-09-14: zero repo-wide, from 35. The sweep also found that the measurement itself was wrong — run the `awk` one file at a time, since mawk attributes a run to whichever `FILENAME` is current when the print fires, and add an `END` block or a run at end-of-file never prints. Recorded in `general.md`. Previously: **Open, and moved.** Repo-wide **30**. `routes/web.php` 14/4/4, `routes/tenant.php` 13/11/8/5, `config/numerosis.php` 5/4/4 all persist. New since this plan: `packages/ui` blade components (12/12/12/7/6/4/4), `database/factories/Tenant/UserFactory` 8/4, `Central/TenantFactory` 8/5, and four in `src/` (`NumerosisServiceProvider:133` 6, `TenancyServiceProvider:304` 5, `AuthenticateLoginCandidate:145` 5, `DefaultUnpaidTenantQuota:30` 4) |
| 6.3 stale symbols | **Closed** 2026-09-14. The `routes/tenant.php` and `routes/web.php` panel and `Socialite\Login` mentions went with the 6.2 rewrites, keeping the invariant and dropping the history, as the phase asked. `InstallNumerosisCommand`'s user-facing failure message and `PromoteFirstCentralUserToAdmin`'s "admin panel" phrasing went too. The invitation migration now names what `SendInvitation` actually does. Previously: **Partial, 2 of 9 left.** Still live: `routes/tenant.php:27` (Filament tenant panel) and `routes/tenant.php:30` (`Socialite\Login`). `routes/web.php:38` also still says "the tenant panel documents 'home' as the one route name guaranteed to exist" — a tenth occurrence this plan missed. The invitations migration `updateOrCreate` comment (`:28`) against `SendInvitation`'s `firstOrNew`+`save` is unchanged. The `docs/architecture.md` and `docs/extending.md` mentions are deliberate records of the deletion, not stale references |
| 7.1 `ModelResolver::modelFor()` | **Fixed** 2026-09-14. `modelFor()` now derives the package model from the factory suffix and hands it to `resolve()`, so the override, the `is_subclass_of` guard and the memoization are one implementation rather than two. Both it and `Numerosis::modelNameFor()` gained a `@template TModel` so a concrete `Factory<Tenant>` passes level 9 — the invariant `Factory<Model>` parameter rejected every real factory class-string. Regression test in `NumerosisSeamTest`, confirmed red (built `App\Models\Central\Tenant` against a configured `Tests\Support\OverriddenTenant`) |
...
| 7.3 small ones | **Closed** 2026-09-14. `SendInvitation::handle()` catches `UniqueConstraintViolationException` and re-issues onto the winning row, the way `EloquentSubscriptionRepository::record()` already did; regression test confirmed red with the literal `1062 Duplicate entry` on `tenant_invitations_tenant_id_email_unique`. **The `HostConfig::set()` dedupe finding is not a defect** and was closed without a change: `apply()` resets `$applied` to `[]` on entry and every key it writes has exactly one writer, so a repeat is unreachable. A probe test asserting `applied()` holds no duplicates passed against the unfixed code, which is why it was not kept |
| 8.1 `PaymentPlanFeatureFactory` | **Fixed** 2026-09-14 by deletion, with the `HasFactory` use and its docblock off `PaymentPlanFeature`. The guard the phase asked for is `tests/Feature/Database/FactoryDefinitionsTest.php`, which walks `database/factories/` and creates one of each; it went red first with the predicted `Unknown column 'feature_key' in 'field list'`. Split in two after the first run: a tenant factory written from the central context throws `ModelNotSyncMasterException` before it reaches a column, so those run inside `TestTenant::provisioned()->run()` |
| 8.2 zero-reference states | **Closed** 2026-09-14. `provisioning()` and `failed()` earned callers during the pipeline work. `unverified()` stays in all three factories, as the phase recommended. `bot()` is deleted per the maintainer's 2026-09-14 decision — it documented a test nobody wrote, and the `is_bot` trap it warned about is already carried by the note in `definition()`. `forCheckout()` stays: it takes a plan and a cycle, so it models a real checkout reservation rather than a bare status |
| 8.3 stub/workbench parity | **Fixed** 2026-09-14. Both missing twins added and `PublishedModelStubsTest` walks every stub, asserting the workbench file exists with identical content. It went red on the missing `Invitation`, and then **turned up a live defect the phase did not predict**: `stubs/Models/Tenant/User.stub` still imported `Policies\UserPolicy`, which the `src/` reorganization moved to `Policies\Auth\UserPolicy` — a host publishing that stub got a class that does not exist. Adding the twins also changed what `Numerosis::model()` resolves for those two models, so two tests registering a `creating` listener on the package class had to move to the resolved one; Eloquent keys listeners by class name and a subclass does not inherit them |
| 8.4 workbench `welcome` view | **Fixed** 2026-09-14, and the 2026-09-07 measurement it rested on no longer holds: `f8a793d` made the harness a real host, so `route:list` is 82 routes with `home`, `get-started` and `billing/webhook` among them. Re-measured on a live boot — the workbench `/` closure is **replaced**, not competing: `RouteCollection` keys by method, domain and URI, and the tenant group's domain-less `/` (`tenant.home`) loads after it. So the dead-code branch is the right one and the route is deleted. The 500 was also not the missing view: `http://127.0.0.1:8000/` fails as `TenantCouldNotBeIdentifiedOnDomainException` because `tenancy.central_domains` is `localhost`. On `http://localhost:8000`, `/` and `/get-started` both answer 200. Both facts recorded in `.ai/rules/package-host-bootstrap.md` |
| 8.5 `rector.php` skip | **Closed** 2026-09-14 as the phase recommended: the entry stays and now says in one line that it guards a path `withPaths()` does not scan yet |
...
| 9.1 `static` widened to `Model` | **Closed** 2026-09-14 by measurement, in favour of the existing code. `static` on both the interface and the implementation is **5 errors cold**, not 0: an interface is not a `Model`, so `@return MorphMany<Subscription, static>` in `Subscribable` resolves to `static(Subscribable)` and `TDeclaringModel` rejects it outright, taking all four implementers with it. The plan's reading — that Larastan objects only on the implementation side — is wrong. Reverted per the phase's own instruction not to iterate on generic variance; all four docblocks now name the real constraint and carry the measurement date |
| 9.2 post-charge `RuntimeException` | **Fixed** 2026-09-14. The same mismatch is knowable before the charge — it is `Cashier::$subscriptionModel`, set once in `BillingServiceProvider::boot()` — so the `throw_unless` moved above `newSubscription()->create()` as an `is_a()` on the class string, and the post-charge narrowing is a bare `assert()`, which PHPStan reads and production does not run. Test asserts the `RuntimeException` arrives with no Stripe credentials configured, which is what proves the ordering |
| 9.3 two fallbacks for one session value | **Fixed** 2026-09-14: the tolerant read wins, since `Plan::cycle()` documents why a stale session must not break the wizard and the payment step is one step further along. `Payment::render()` is `tryFrom() ?? Monthly` now. Test renders the step with `billing_cycle = 'fortnightly'`, confirmed red as a `ValueError` |
| 9.4 `socialiteproviders/manager` | **Fixed** 2026-09-14. `composer.json` requires it directly at `^4.10`. The guard is a new `PackageBoundariesTest` case mapping a vendor namespace core names to the package that must be in `require`, confirmed red against the unfixed `composer.json` |
| 9.5 four copies of `available()` | **Closed.** One copy, in `src/Features/Concerns/IsNamedFeature.php` |
| 9.6 comment regressions | **Closed** with 6.1/6.2, which measured the same files. Previously: **Open,** and folded into the 6.1/6.2 counts above. `RoleAndPermissionSeeder` went 14→14 and 9, `SeedsAdminRole` 9 plus a 6-line `//` run, `SubscriptionFactory` 12 and 9 plus a 6-line run |
| 9.7 naming and placement | **Closed** 2026-09-14: `tenantsMine()`'s parameter is `$key`, `TestCase`'s MySQL host/port/user/password are four constants read by both the connection array and the `CREATE DATABASE`, and `numerosis.tenancy.implementations` says why the three auth contracts sit there (`TenancyServiceProvider` binds the map, and login resolves a candidate across both databases). Previously: **1 of 4.** `HostConfig::filesystemDisks()` now *removes* `livewire` from the tenant-suffixed disk list rather than appending it, and its docblock matches, so that item is moot. Still open: `CompleteRedirectCheckout::tenantsMine(string $level, string $message)` where `$message` is a translation key; `numerosis.tenancy.implementations` (`config/numerosis.php:357`) still binding four non-tenancy auth contracts; `TestCase::ensureDatabaseExists()` still hardcoding a second copy of the MySQL credentials at `tests/TestCase.php:579` |
...
**Closed 2026-09-14.** `f8a793d` named `NumerosisServiceProvider` in `testbench.yaml`, and `NumerosisServiceProvider::registerRoutesFallback()` registers the routes for any app whose `bootstrap/app.php` did not — so the harness now carries all of them. The finding below described the state before that commit.
...
### 10.3 `LoginWithSocialAccount`'s transaction is on the wrong connection

Found 2026-09-07 while fixing the same defect in `AcceptInvitation` (Phase 3),
and left alone because it is outside that phase.

`LoginWithSocialAccount::handle()` wraps its two writes in `DB::transaction()`,
which opens on `database.default`. `CentralUser` and `SocialAccount` are both
`CentralConnection` models on the `central` connection — a different
`Connection` object with its own PDO handle and transaction stack. The
transaction therefore covers neither write.

Its own comment states what that costs: "Split, a failure on the second left a
passwordless `CentralUser` with no credential, which the `$hasLocalAccount`
branch then refused on every retry." That is the live behaviour today; the
comment describes protection the code does not have.

One line — `$centralUserClass::query()->getConnection()->transaction(...)`, or
resolve the connection the way `AcceptInvitation` now does. The test is a
failure injected into `createSocialAccount()` plus an assertion that no
`users` row survives. `grep -rn 'DB::transaction' src/` finds no third site.
