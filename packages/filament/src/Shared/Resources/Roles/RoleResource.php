...
namespace Nvade\NumerosisFilament\Shared\Resources\Roles;
...
/**
 * The full role-management UI, subclassed by the tenant panel's Team cluster.
 *
 * `Shared\` is not a panel — it sits beside `Admin\` and `TenantAdmin\`, which
 * are. It was called `App\` until 2026-09-01, which read as a third panel and
 * collided with the meaning `App\` carries in every Laravel host.
 *
 * The central panel deliberately does **not** extend this.
 * `Admin\Resources\Roles\RoleResource` is a separate, much smaller screen
 * because it answers a different question: staff auditing which roles exist,
 * rather than a tenant owner composing permission sets for their own team.
 * Two classes sharing a basename is the cost; merging them would mean handing
 * one audience the other's UI. Change permission semantics and you must check
 * both.
 */
