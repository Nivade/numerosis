...
At session start, read `.claude/rules/INDEX.md`. Indexes non-obvious, learned facts about this codebase (race conditions, invariants spanning multiple files, "why built this way") organized by topic in `.claude/rules/*.md`. Open any topic file relevant to area you about to touch before changing there.

## Plan Mode Output

After plan-mode plan approved, copy plan file into this repo `.claude/plans/` folder (create if missing) using same filename, so plans versioned with project instead of only living in user home directory.

## Skills & Rules Activation

- **Rules Directory:** This project has strict development rules located in `.claude/rules/`. You MUST read, parse, and adhere to all rule files within that directory before starting any task or
  generating code.
- Use `codebase-learnings` skill to add to these rules whenever you or user surface non-obvious fact worth not re-discovering — see that skill for what belongs here vs. personal memory.
- **Domain Skills:** This project has domain-specific skills available in `**/skills/**`. You MUST activate the relevant skill whenever you work in that domain—don't wait until you're stuck.

## Critical Execution Constraints

- **FORBID SUB-AGENTS:** NEVER spawn, invoke, or initialize sub-agents, child agents, or background instances under any circumstances.
- **Single Session Execution:** ALL tasks, tool usage, file operations, and terminal executions must happen directly within this single, main active session to conserve tokens.
- **User Delegation:** If a task feels too large or complex for a single thread, do not delegate it to a sub-agent. Instead, stop and ask the user to break the request down into smaller, sequential
  steps.
