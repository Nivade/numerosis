...
| `php artisan filament:assets` | run at least once (already required for Filament's own core CSS) | Both Filament panels' theming (`NumerosisAdminPlugin`/`NumerosisTenantPlugin`'s `->theme(NumerosisServiceProvider::THEME_ID)`) comes from a prebuilt CSS file registered as a Filament `Theme` asset in `NumerosisServiceProvider::registerFilamentTheme()` — the same mechanism `filament/filament` uses for its own default theme (`vendor/filament/filament/dist/theme.css`). Nothing to add to `vite.config.js`: `filament:assets` copies `dist/filament-theme.css` straight to `public/css/nvade/numerosis/`, bypassing the host's build entirely. A host that has never run `filament:assets` gets no error, just an unstyled panel — same failure shape Filament's own core CSS has if that command is skipped. | `verifyFilamentThemeAsset()` |
