...
| `ActivityLogFeature` | `activity_log` | The screens that read the audit log: `team/activity` for a tenant's owner and admins, the staff feed at `<staff_prefix>/activity`, and the recent-activity card on the staff tenant detail | Writing entries, which is unconditional. `activitylog.enabled` is the switch for that, and a log with holes in it answers nothing |
...
| `BillingNotificationsFeature` note | — | Still gates the *billing* notifications specifically. Everything else about delivery — in-app copy, per-user channel preferences, the unsubscribe link — is `numerosis.notifications.*` and not a feature flag |
| `UsageMeteringFeature` | `usage_metering` | **Off by default.** The tenant-facing usage screen at `usage`, showing each meter the plan declares in `metadata.options.meters` against its included allowance for the current billing period | Everything that bills: `billing:report-usage` sends meter events and `billing:reconcile-usage` checks them whether or not this is on, both gated by `numerosis.schedule.*` instead. Hiding the screen never stops the meter |
...
| `ReadApiFeature` | `read_api` | **Off by default.** The read-only `/api/v1` surface (tenant, members, invitations, subscription, domains) and the `api-tokens` screen that mints Sanctum keys for it | Existing tokens keep working against whatever routes are registered; with this off, nothing is |
| `NotificationCenterFeature` | `notification_center` | The bell in the header and its panel | Notifications are still written; only the reading UI disappears |
| `NotificationPreferencesFeature` | `notification_preferences` | The `settings/notifications` channel matrix | Existing preference rows, and the defaults each notification falls back to |
