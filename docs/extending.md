...
- **Only write config in the register phase where the parent namespace is
  deep-filled.** A namespace `HostConfig::numerosisConfig()` deep-fills
  survives a `packageRegistered()` write. `numerosis.tenancy`
...
- **One `class_exists()` seam per optional package, not one per call site.**
  Give the optional dependency exactly one `available()` predicate and have
  every consumer ask it. One unguarded call site is a fatal, not a disabled
  feature. `spatie/laravel-activitylog` and `spatie/laravel-one-time-passwords`
  are the remaining cases.

## Where a package's own pieces already live

Every migration stays in core: the tables have to exist wherever core does.
...
"which package added this route" is answerable only by grep. `numerosis-account`
landed without them; add the readers alongside the writers before the next
package does.
